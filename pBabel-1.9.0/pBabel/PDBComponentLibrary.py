...
from pMolecule                 import DoubleBond, SingleBond

from PDBComponent              import PDBComponent, PDBComponentAtom, PDBComponentBond, PDBComponentLink, PDBComponentVariant
...
# . Class.
#===================================================================================================================================
class PDBComponentLibrary ( object ):
    ...
    def __init__ ( self, paths = None, readOnly = True ):
        """Constructor."""
        for ( key, value ) in self.__class__.defaultAttributes.iteritems ( ): setattr ( self, key, value )
        self.DefineLibraryPaths ( paths )
        self.SetupPaths ( makePaths = not readOnly )
        self.ResetCache ( )
    ...
    def MakeStandardLibraryPath ( self ):
        ...
        return os.path.join ( os.getenv ( "PDYNAMO_PARAMETERS" ), _LibraryPath )
...
def _MakeDefaultLinks ( ):
    ...
    links = []
    # . Disulfide bridge.
    variant = PDBComponentVariant ( atomsToDelete = [ "HG" ], component = "CYS" )
    links.append ( PDBComponentLink ( label = "Disulfide Bridge", leftAtomLabel = "SG", leftVariant = variant, rightAtomLabel = "SG", rightVariant = variant, bondType = SingleBond ( ) ) )
    # . Peptide bonds.
    # . The left variant is always the same.
    leftVariant = PDBComponentVariant ( atomsToDelete = [ "HXT", "OXT" ] )
    # . General case, PRO and UNK.
    rightVariant = PDBComponentVariant ( atomsToDelete = [ "H2" ] )
    links.append ( PDBComponentLink ( label = "Peptide", leftAtomLabel = "C", leftVariant = leftVariant, rightAtomLabel = "N", rightVariant = rightVariant, bondType = SingleBond ( ) ) )
    rightVariant = PDBComponentVariant ( atomsToDelete = [ "H"  ], componentLabel = "PRO" )
    links.append ( PDBComponentLink ( label = "Peptide", leftAtomLabel = "C", leftVariant = leftVariant, rightAtomLabel = "N", rightVariant = rightVariant, bondType = SingleBond ( ) ) )
    rightVariant = PDBComponentVariant ( atomsToDelete = [ "H2" ], componentLabel = "UNK" )
    links.append ( PDBComponentLink ( label = "Peptide", leftAtomLabel = "C", leftVariant = leftVariant, rightAtomLabel = "N", rightVariant = rightVariant, bondType = SingleBond ( ) ) )
    # . Finish up.
    return links
...
def _MakeDefaultVariants ( ):
    ...
    variants = []
    # . CTerminal.
    variants.append ( PDBComponentVariant ( label = "C Terminal", atomsToDelete = [ "HXT" ], formalCharges = { "OXT" : -1 } ) )
    # . NTerminal - general case and UNK.
    atomsToAdd = [ PDBComponentAtom ( atomicNumber = 1, label = "H1", pdbAlign = 0, toFollow = "N"  ),
                   PDBComponentAtom ( atomicNumber = 1, label = "H2", pdbAlign = 0, toFollow = "H1" ),
                   PDBComponentAtom ( atomicNumber = 1, label = "H3", pdbAlign = 0, toFollow = "H2" )  ]
    bondsToAdd = [ PDBComponentBond ( atomLabel1 = "H1", atomLabel2 = "N", bondType = SingleBond ( ) ),
                   PDBComponentBond ( atomLabel1 = "H2", atomLabel2 = "N", bondType = SingleBond ( ) ),
                   PDBComponentBond ( atomLabel1 = "H3", atomLabel2 = "N", bondType = SingleBond ( ) )  ]
    variants.append ( PDBComponentVariant (                         label = "N Terminal", atomsToAdd = atomsToAdd, atomsToDelete = [ "H", "H2" ], formalCharges = { "N" : +1 }, bondsToAdd = bondsToAdd ) )
    variants.append ( PDBComponentVariant ( componentLabel = "UNK", label = "N Terminal", atomsToAdd = atomsToAdd, atomsToDelete = [ "H", "H2" ], formalCharges = { "N" : +1 }, bondsToAdd = bondsToAdd ) )
    # . NTerminal - PRO.
    atomsToAdd = [ PDBComponentAtom ( atomicNumber = 1, label = "H2", pdbAlign = 0, toFollow = "N"  ),
                   PDBComponentAtom ( atomicNumber = 1, label = "H3", pdbAlign = 0, toFollow = "H2" )  ]
    bondsToAdd = [ PDBComponentBond ( atomLabel1 = "H2", atomLabel2 = "N", bondType = SingleBond ( ) ),
                   PDBComponentBond ( atomLabel1 = "H3", atomLabel2 = "N", bondType = SingleBond ( ) )  ]
    variants.append ( PDBComponentVariant ( componentLabel = "PRO", label = "N Terminal", atomsToAdd = atomsToAdd, atomsToDelete = [ "H" ], formalCharges = { "N" : +1 }, bondsToAdd = bondsToAdd ) )
    # . Deprotonated ASP and GLU.
    variants.append ( PDBComponentVariant ( componentLabel = "ASP", label = "Deprotonated", atomsToDelete = [ "HD2" ], formalCharges = { "OD2" : -1 } ) )
    variants.append ( PDBComponentVariant ( componentLabel = "GLU", label = "Deprotonated", atomsToDelete = [ "HE2" ], formalCharges = { "OE2" : -1 } ) )
    # . Histidine variants.
    # . Doubly protonated - leave as is but shift charge to Nepsilon.
    bondTypes = [ PDBComponentBond ( atomLabel1 = "CE1", atomLabel2 = "ND1", bondType = SingleBond ( ) ),
                  PDBComponentBond ( atomLabel1 = "CE1", atomLabel2 = "NE2", bondType = DoubleBond ( ) ) ]
    variants.append ( PDBComponentVariant ( componentLabel = "HIS", label = "Doubly Protonated",                             formalCharges = { "ND1" : 0, "NE2" : +1 }, bondTypes = bondTypes ) )
    # . Delta protonated.
    bondTypes = [ PDBComponentBond ( atomLabel1 = "CE1", atomLabel2 = "ND1", bondType = SingleBond ( ) ),
                  PDBComponentBond ( atomLabel1 = "CE1", atomLabel2 = "NE2", bondType = DoubleBond ( ) ) ]
    variants.append ( PDBComponentVariant ( componentLabel = "HIS", label = "Delta Protonated",   atomsToDelete = [ "HE2" ], formalCharges = { "ND1" : 0, "NE2" :  0 }, bondTypes = bondTypes ) )
    # . Epsilon protonated.
    variants.append ( PDBComponentVariant ( componentLabel = "HIS", label = "Epsilon Protonated", atomsToDelete = [ "HD1" ], formalCharges = { "ND1" : 0, "NE2" :  0 } ) )
    # . Fully deprotonated.
    variants.append ( PDBComponentVariant ( componentLabel = "HIS", label = "Fully Deprotonated", atomsToDelete = [ "HD1", "HE2" ], formalCharges = { "ND1" : 0, "NE2" : -1 } ) )
    # . Finish up.
    return variants
...
def MakeDefaultPDBComponentLibrary ( cifPath = None, fullLibrary = False, log = logFile, libraryPaths = None, outPath = None ):
    ...
    # . Get the cifPath.
    if cifPath is None:
        cifPath = os.path.join ( os.getenv ( "PDYNAMO_PARAMETERS" ), _LibraryPath, _DefaultCIFFileName )
    # . Get the components.
    components = PDBComponentCIFFile_ToComponents ( cifPath, asDictionary = True, log = log )
    ...
    # . Create the library.
    library = PDBComponentLibrary ( paths = libraryPaths, readOnly = False )
