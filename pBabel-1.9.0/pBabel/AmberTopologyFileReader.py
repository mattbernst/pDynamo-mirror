#-------------------------------------------------------------------------------
# . File      : AmberTopologyFileReader.py
...
from pCore        import Clone, logFile, LogFileActive, SelfPairList_FromIntegerPairs, TextFileReader, UNITS_ENERGY_KILOCALORIES_PER_MOLE_TO_KILOJOULES_PER_MOLE
from ExportImport import _Importer
from pMolecule    import CrystalClassCubic, CrystalClassMonoclinic, CrystalClassOrthorhombic, CrystalClassTetragonal, FourierDihedralContainer, HarmonicAngleContainer, HarmonicBondContainer, \
                         LJParameterContainer_FromTableCoefficients, MMAtomContainer, MMModelAMBER, MMTermContainer, PeriodicTable, Sequence, System
...
_SECTIONDATA = { "ATOM_NAME"                  : ( _NATOMS, _YX         ) ,
                 ...
                 "SCEE_SCALE_FACTOR"          : ( -1    ,  _YX         ) ,
                 "SCNB_SCALE_FACTOR"          : ( -1    ,  _YX         ) ,
                 "SOLTY"                      : ( _NATYP,  _YX         ) ,
...
class AmberTopologyFileReader ( TextFileReader ):
    ...
    defaultattributes = { "QUNTANGLE" : False, \
                          "QVERIFIED" : False  }
            ...
            for i in range ( self.pointers[_NATOMS] ):
                ...
                for j in self.excluded_atoms_list[nnb:nnb+n]:
                    if j > 0:
                        ...
                        ijex.append ( j - 1 )
                nnb += n
            mm = SelfPairList_FromIntegerPairs ( ijex )
            mm.label = "Exclusions"
        return mm
    ...
    def ToLJParameterContainer ( self, analyticForm ):
        ...
        if ntypes > 0: mm = LJParameterContainer_FromTableCoefficients ( ntypes, self.lennard_jones_acoef, self.lennard_jones_bcoef, analyticForm = analyticForm )
        return mm

    def ToMMAtomContainer ( self ):
        """Create a MM atom container."""
        mm     = None
        natoms = self.pointers[_NATOMS]
        if natoms > 0:
            ...
            # . Construct atom data.
            atoms = []
            for ( aType, ljType, charge ) in zip ( indices, self.atom_type_index, self.charge ): 
                atoms.append ( ( True, aType, ljType - 1, charge ) )
            # . Create the container.
            state = { "atoms" : atoms, "atomTypes" : atomTypes }
    ...
    def UntangleDihedralsAnd14s ( self ):
        ...
        if not self.QUNTANGLE:
            ...
            data        = self.dihedrals_inc_hydrogen + self.dihedrals_without_hydrogen
            ...
            if ij14 is not None:
                self.interactions14 = SelfPairList_FromIntegerPairs ( ij14 )
    ...
    def Verify ( self ):
        ...
        if self.QPARSED and not self.QVERIFIED:
            # . Charges.
