...
from pCore        import logFile, LogFileActive, SelfPairList_FromIntegerPairs, Selection, TextFileReader
...
from pMolecule    import CMAPDihedralContainer, FourierDihedralContainer, HarmonicAngleContainer, HarmonicBondContainer, HarmonicImproperContainer, \
                         LJParameterContainer_FromEpsilonSigma, MMAtomContainer, MMModelCHARMM, MMTermContainer, PeriodicTable, Sequence, System
...
#                                print self.counters
    ...
    def Summary ( self, log = logFile ):
        ...
        if self.QPARSED and LogFileActive ( log ):
            ...
            keys.sort ( )
            for key in keys: summary.Entry ( key, "{:d}".format ( self.counters[key] ) )
            summary.Stop ( )
    ...
    def ToFixedAtoms ( self, system ):
        ...
        if natoms > 0:
            ...
            if len ( indices ) > 0: system.DefineFixedAtoms ( Selection.FromIterable ( indices ) )
    ...
    def ToLJParameterContainers ( self, parameters = {}, parametersWild = {}, parameters14 = {}, parameters14Wild = {} ):
        ...
        if ntypes > 0:
            ...
            ljParameters   = LJParameterContainer_FromEpsilonSigma ( ntypes, epsilons  , sigmas  , analyticForm = "AMBER", parameterKeys = self.uniqueAtomTypes )
            ljParameters14 = LJParameterContainer_FromEpsilonSigma ( ntypes, epsilons14, sigmas14, analyticForm = "AMBER", parameterKeys = self.uniqueAtomTypes )
    ...
    def ToSystem ( self, mmModel = None, parameters = None ):
        ...
        if self.QPARSED:
            ...
            if ( mmModel is not None ) and ( parameters is not None ) and ( self.isXPLOR ):
                ...
                if ljParameters14 is not None: system.energyModel.ljParameters14 = ljParameters14
...
# . Helper functions.
#===================================================================================================================================
def CHARMMPSFFile_ToSystem ( filename, isXPLOR = False, log = logFile, mmModel = None, parameters = None ):
...
_Importer.AddHandler ( { System : CHARMMPSFFile_ToSystem } , [ "psf", "psfx", "PSF", "PSFX" ], "Charmm PSF", defaultFunction = CHARMMPSFFile_ToSystem )
