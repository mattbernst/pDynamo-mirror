...
from pCore     import logFile, LogFileActive, SelfPairList_FromIntegerPairs, TextFileReader, UNITS_ENERGY_KILOCALORIES_PER_MOLE_TO_KILOJOULES_PER_MOLE
from pMolecule import CMAPDihedralContainer, FourierDihedralContainer, HarmonicAngleContainer, HarmonicBondContainer, HarmonicImproperContainer, \
                      LJParameterContainer_FromEpsilonSigma, MMAtomContainer, MMModelCHARMM, MMModelAMBER, MMModelOPLS, MMTermContainer, \
                      PeriodicTable, Sequence, System
...
#===================================================================================================================================
class GromacsParameterContainer ( object ):
...
class GromacsFileReader ( TextFileReader ):
    """Class for general Gromacs topology files that are to be read."""
    ...
    def Parse ( self, log = logFile, parameters = None ):
        ...
        if not self.QPARSED:
		    ...
		    # . Error or Data from previously found section.
		        ...
		        if self.__class__.__name__ == "GromacsDefinitionsReader": self.ProcessMoleculeType ( current )
                        current = section
			# . Include data used to identify dihedral type.
                ...
                pass
            # . Process the data.
            if self.__class__.__name__ == "GromacsParameterReader": 
...
#===================================================================================================================================
class GromacsParameterReader ( GromacsFileReader ):
    ...
    def Summary ( self, log = logFile ):
        ...
        if self.QPARSED and LogFileActive ( log ):
            ...
            summary.Start ( "Gromacs Topology Section Lines Summary" )
	    inverted = {}
            items = _SectionMapping.items ( )
	    for ( key, value ) in items: inverted[value] = key
	    for section in self.topData.keys ( ):
	        if len ( self.topData[section] ) > 0:
	            summary.Entry ( inverted[section], "{:d}".format ( len ( self.topData[section] ) ) )
            summary.Stop ( )
            # . Processed data.
            summary = log.GetSummary ( )
            summary.Start ( "Gromacs Topology Parameter Summary" )
            summary.Entry ( "Angles"              , "{:d}".format ( len ( self.angle        ) ) )
            summary.Entry ( "Bonds"               , "{:d}".format ( len ( self.bond         ) ) )
            summary.Entry ( "Cmaps"               , "{:d}".format ( len ( self.cmap         ) ) )
            summary.Entry ( "Dihedrals (Nonwild)" , "{:d}".format ( len ( self.proper       ) ) )
            summary.Entry ( "Dihedrals (Wild)"    , "{:d}".format ( len ( self.properwild   ) ) )
            summary.Entry ( "Impropers (Nonwild)" , "{:d}".format ( len ( self.improper     ) ) )
            summary.Entry ( "Impropers (Wild)"    , "{:d}".format ( len ( self.improperwild ) ) )
            summary.Entry ( "Nonbonds"            , "{:d}".format ( len ( self.nonbond      ) ) )
            summary.Entry ( "Nonbonds 1-4"        , "{:d}".format ( len ( self.nonbond14    ) ) )
            summary.Entry ( "Urey-Bradleys"       , "{:d}".format ( len ( self.ureybradley  ) ) )
            summary.Stop ( )
...
#===================================================================================================================================
class GromacsDefinitionsReader ( GromacsFileReader ):
    ...
    def Summary ( self, log = logFile ):
        ...
        if LogFileActive ( log ):
            # . Processed data.
            summary = log.GetSummary ( )
            summary.Start ( "Gromacs Topology Definitions Summary" )
            summary.Entry ( "Angles"              , "{:d}".format ( len ( self.angles     ) // 3 ) )
            summary.Entry ( "Atoms"               , "{:d}".format ( len ( self.atoms      )     ) )
            summary.Entry ( "Bonds"               , "{:d}".format ( len ( self.bonds      ) // 2 ) )
            summary.Entry ( "CMaps"               , "{:d}".format ( len ( self.cmaps      ) // 5 ) )
            summary.Entry ( "Dihedrals"           , "{:d}".format ( len ( self.dihedrals  ) // 4 ) )
            summary.Entry ( "Exclusions 1-4"      , "{:d}".format ( len ( self.exclusions ) // 2 ) )
            summary.Entry ( "Impropers"           , "{:d}".format ( len ( self.impropers  ) // 4 ) )
            summary.Entry ( "MoleculeTypes"       , "{:d}".format ( len ( self.parameters.molecules ) ) )
            summary.Stop ( )
    ...
    def ToExclusionPairLists ( self ):
        ...
        if nbonds > 0 :
            ...
            if len ( i1234 ) > 0:
                ...
                exclusions = SelfPairList_FromIntegerPairs ( i1234 )
                exclusions.label = "Exclusions"
            if len ( i14 ) > 0:
                i14 = list ( itertools.chain ( * list ( i14 ) ) )
                interactions14 = SelfPairList_FromIntegerPairs ( i14 )
    ...
    def ToLJParameterContainers ( self, parameters = {}, parameters14 = {} ):
        ...
        if ntypes > 0:
            ...
            ljParameters   = LJParameterContainer_FromEpsilonSigma ( ntypes, epsilons  , sigmas  , analyticForm = "AMBER" )
            ljParameters14 = LJParameterContainer_FromEpsilonSigma ( ntypes, epsilons14, sigmas14, analyticForm = "AMBER" )
            ljParameters14.label = "1-4 LJ Parameters"
        return ( ljParameters, ljParameters14 )

    def ToMMAtomContainer ( self ):
        """Create a MM atom container."""
        mm     = None
	if self.natoms > 0 :
            # . Gather some data.
            atomTypes = []
            ...
            # . Create the atom data.
            atoms = []
            for ( atomType, charge ) in zip ( atomTypes, charges ):
                index = uniqueAtomTypeIndex[atomType]
                atoms.append ( ( True, index, index, charge ) )
            # . Save the atom types.
            self.typesOfAtoms    = atomTypes
            self.uniqueAtomTypes = uniqueAtomTypes
            # . Use the atomType indices as the LJ indices too.
            # . Create the container.
            state = { "atoms" : atoms, "atomTypes" : uniqueAtomTypes }
    ...
    def ToSystem ( self, parameters = None ):
	    ...
	    if   self.ff == "AMBER": mmModel = MMModelAMBER  ( )
            ...
            if ( mmModel is not None ) and ( parameters is not None ):
		   ...
		   mm   = self.ToFourierImproperContainer     ( parameters = parameters.improper, parameterswild = parameters.improperwild )
                ...
                if exclusions     is not None: system.energyModel.exclusions     = exclusions
                if interactions14 is not None: system.energyModel.interactions14 = interactions14
                # . LJ terms.
                ( ljParameters, ljParameters14 ) = self.ToLJParameterContainers ( parameters = parameters.nonbond, parameters14 = parameters.nonbond14 )
                if ljParameters   is not None: system.energyModel.ljParameters = ljParameters
...
# . Helper functions.
#===================================================================================================================================
def GromacsDefinitions_ToSystem ( filename, log = logFile, parameters = None ):
    """Helper function that generates a system from a Gromacs file."""
    infile = GromacsDefinitionsReader ( filename )
    infile.Parse             ( log = log, parameters = parameters )
    system = infile.ToSystem (            parameters = parameters )
    infile.Summary           ( log = log )
    system.sequence.PrintComponentSequence ( log = log )
    return system

def GromacsParameters_ToParameters ( filename, log = logFile ):
    """Return the parameters from a list of parameter files."""
    # . Get the parameter set.
    parameterfile = GromacsParameterReader ( filename )
    parameterfile.Parse   ( log = log )
    parameterfile.Summary ( log = log )
    parameters    = parameterfile.ToParameters ( ) 
    return parameters
