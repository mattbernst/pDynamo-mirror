...
from Units       import CONSTANT_BOLTZMANN, CONSTANT_PLANCK, CONSTANT_SPEED_OF_LIGHT

# . Units are not currently treated so any conversions need to be handled explicitly.

# . Surface temperature of the sun in K.
CONSTANT_SUN_SURFACE_TEMPERATURE = 5778.0
...
class DataRangeIterator ( object ):
...
class DataSet ( object ):
    ...
    def WithSize ( selfClass, size ):
        ...
        self      = selfClass ( )
        self.data = Real1DArray.WithExtent ( size )
        return self
...
class Spectrum ( object ):
    ...
    defaultAttributes = { "abscissae" : None ,
                          "label"     : None ,
                          "ordinates" : None }
...
class ContinuousSpectrum ( Spectrum ):
    ...
    def spline ( self ):
        ...
        if result is None:
            result = CubicSpline.FromFunctionValues ( self.ordinates.data, x = self.abscissae.data )
...
class BlackBodySpectrum ( ContinuousSpectrum ):
    ...
    defaultAttributes = { "temperature" : CONSTANT_SUN_SURFACE_TEMPERATURE }
    defaultAttributes.update ( ContinuousSpectrum.defaultAttributes )

    def SetOrdinates ( self ):
        """Set the ordinate values."""
        a     = ( CONSTANT_PLANCK * CONSTANT_SPEED_OF_LIGHT) / ( CONSTANT_BOLTZMANN * self.temperature )
        b     = ( 2.0 * CONSTANT_PLANCK * CONSTANT_SPEED_OF_LIGHT**2 )
        nmToM = 1.0e-9
    ...
    @classmethod
    def WithAbscissae ( selfClass, abscissae, temperature = CONSTANT_SUN_SURFACE_TEMPERATURE ):
