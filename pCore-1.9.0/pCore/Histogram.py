...
# . Histogram class.
#-----------------------------------------------------------------------------------------------------------------------------------
class RegularHistogram ( object ):
    """A class for regular histograms of arbitrary dimension."""

    defaultAttributes = { "bins"               : 0    ,
                          "counts"             : None ,
                          "dimensions"         : None ,
...
#            if index >= 0: self.counts[index] += 1
    ...
    def Normalize ( self, bindata ):
        ...
        if isinstance ( bindata, Real1DArray ) and ( len ( bindata ) == self.bins ):
            volume = sum ( bindata ) * self.integrationElement
            bindata.Scale ( 1.0 / volume )
        else:
            raise TypeError ( "Invalid bin data argument of type {!r} and length {:d}.".format ( type ( bindata ), len ( bindata ) ) )
    ...
    def ReduceData ( self, toreduce, bindata ):
        ...
        for ( i, dimension ) in enumerate ( self.dimensions ):
            if i not in toremove:
                ...
                tokeep.append ( i )
        # . Allocate space.
        if isinstance ( bindata, Real1DArray ):
            new = Real1DArray.WithExtent ( bins )
            new.Set ( 0.0 )
        else:
            new = bins * [ 0 ]
...
# . Iterator class.
#-----------------------------------------------------------------------------------------------------------------------------------
class RegularHistogramBinMidPointIterator ( object ):
...
# . Iterator class.
#-----------------------------------------------------------------------------------------------------------------------------------
def RegularHistogramDataIndexIterator ( object ):
...
#-----------------------------------------------------------------------------------------------------------------------------------
class RegularHistogramDimension ( object ):
...
# . Iterator class.
#-----------------------------------------------------------------------------------------------------------------------------------
def RegularHistogramIndexIterator ( object ):
