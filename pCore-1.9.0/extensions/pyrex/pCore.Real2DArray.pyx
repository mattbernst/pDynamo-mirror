#-------------------------------------------------------------------------------
# . File      : pCore.Real2DArray.pyx
# . Program   : pDynamo-1.9.0                           (http://www.pdynamo.org)
# . Copyright : CEA, CNRS, Martin J. Field (2007-2014)
# . License   : CeCILL French Free Software License     (http://www.cecill.info)
#-------------------------------------------------------------------------------
"""Handle real 2-D arrays."""
...
cdef class Real2DArray:
    ...
    def __dealloc__ ( self ):
        """Finalization."""
        if self.isOwner:
            Real2DArray_Deallocate ( &self.cObject )
            self.isOwner = False
        self.owner = None

    def __deepcopy__ ( self, memo ):
        """Copying."""
        cdef Real2DArray new
    ...
    def __getitem__ ( self, indices ):
        ...
        if view is None:
            ...
            value      = Real2DArray_GetItem ( self.cObject, i, j, &status )
        ...
        if self.isOwner:
            ...
            for i from 0 <= i < extent0:
                for j from 0 <= j < extent1:
                    items.append ( Real2DArray_GetItem ( self.cObject, i, j, NULL ) )
    ...
    def __setitem__ ( self, indices, Real value ):
        ...
        if view is None:
            ...
            j          = int ( indices[1] )
            status     = Status_Continue
            Real2DArray_SetItem ( self.cObject, i, j, value, &status )
            if status != Status_Continue: raise IndexError ( "Indices ({:d},{:d}) out of range.".format ( i, j ) )
        else:
            view.Set ( value )
    ...
    def __setstate__ ( self, state ):
        ...
        if items is not None:
            ...
            for i from 0 <= i < extent0:
                for j from 0 <= j < extent1:
                    value = items[n]
                    Real2DArray_SetItem ( self.cObject, i, j, value, NULL )
                    n += 1
        else:
            offset               = state["offset" ]
            owner                = state["owner"  ]
    ...
    def _Allocate ( self, Integer extent0, Integer extent1 ):
        """Allocation."""
        cdef Status status
        status       = Status_Continue
        self.cObject = Real2DArray_Allocate ( extent0, extent1, &status )
        if status   != Status_Continue: raise ValueError ( "C object allocation failure." )
        self.isOwner = True
        self.owner   = None
    ...
    def _Slice1D ( self, i, j ):
        ...
        view.owner   = self
        status       = Status_Continue
        Real2DArray_1DSlice ( self.cObject, start0, stop0, stride0, start1, stop1, stride1, view.cObject, &status )
    ...
    def _Slice2D ( self, i, j ):
        ...
        view.owner   = self
        status       = Status_Continue
        Real2DArray_Slice ( self.cObject, start0, stop0, stride0, start1, stop1, stride1, view.cObject, &status )
        if status   != Status_Continue: raise CLibraryError ( "Invalid 2-D slice." )
        return view

    def _ViewOf1DArray ( self, Real1DArray other, Integer offset, Integer extent0, Integer stride0, Integer extent1, Integer stride1 ):
        """Make a 2D view of a 1D array."""
        cdef Status status
        status = Status_Continue
        Real2DArray_ViewOfRaw ( self.cObject, offset, extent0, stride0, extent1, stride1, other.cObject.data, other.cObject.size, &status )
        if status != Status_Continue: raise CLibraryError ( "Error creating view." )
        self.isOwner = False
        self.owner   = other

    def CopyTo ( self, Real2DArray other ):
        """Copying."""
        Real2DArray_CopyTo ( self.cObject, other.cObject, NULL )

    def Determinant ( self ):
        """Calculate the determinant of a square matrix."""
        if self.isSquare:
            return Real2DArray_Determinant ( self.cObject, NULL )
        else:
            raise ValueError ( "Cannot calculate the determinant of a non-square matrix." )

    def Exp ( self ):
        """Exponentiate the elements."""
        Real2DArray_Exp ( self.cObject )

    def Extent ( self, Integer dimension ):
        """Return the extent of an array dimension."""
        return Real2DArray_Length ( self.cObject, dimension )
    ...
    def MatrixMultiply ( self, Real2DArray x, Real2DArray y, alpha = 1.0, beta = 0.0, xTranspose = False, yTranspose = False ):
        """Calculate self -> beta * self + alpha * x * y. x and y can be transposed."""
        cdef Boolean transposeX, transposeY
        if xTranspose: transposeX = CTrue
        else:          transposeX = CFalse
        if yTranspose: transposeY = CTrue
        else:          transposeY = CFalse
        Real2DArray_MatrixMultiply ( transposeX, transposeY, alpha, x.cObject, y.cObject, beta, self.cObject, NULL )

    def Print ( self, columnLabels = None , indexWidth = 6    , itemFormat = "{:18.8f}", itemsPerRow =  6   ,
                      itemWidth    = 19   , rowLabels  = None , log        = logFile , title       = None ):
        """Printing."""
        if LogFileActive ( log ):
            ...
            for itable in range ( ntables ):
                ...
                for irow in range ( numberRows ):
                    ...
                    for i in range ( nelements ): table.Entry ( itemFormat.format ( Real2DArray_GetItem ( self.cObject, irow, i + itable * itemsPerRow, NULL ) ) )
                table.Stop ( )

    def ProjectOutOfArray ( self, Real1DArray vector ):
        """Project the array out of another one."""
        Real2DArray_ProjectOutOf1DArray ( self.cObject, vector.cObject, NULL )
...
#        return Real2DArray_RootMeanSquare ( self.cObject )

    def Scale ( self, Real value ):
        """Scale the elements."""
        Real2DArray_Scale ( self.cObject, value )

    def Set ( self, Real value ):
        """Set the elements."""
        Real2DArray_Set ( self.cObject, value )

    def Transpose ( self ):
        """Transpose the array."""
        Real2DArray_Transpose ( self.cObject, NULL )

    def VectorMultiply ( self, Real1DArray x, Real1DArray y, alpha = 1.0, beta = 0.0, transpose = False ):
        """Calculate y -> beta * y + alpha * self * x. self can be transposed."""
        cdef Boolean aTranpose
        if transpose: aTranspose = CTrue
        else:         aTranspose = CFalse
        Real2DArray_VectorMultiply ( aTranspose, alpha, self.cObject, x.cObject, beta, y.cObject, NULL )
    ...
    # . Properties.
    property columns:
        def __get__ ( self ): return Real2DArray_Length ( self.cObject, 1 )
    ...
    property rows:
        def __get__ ( self ): return Real2DArray_Length ( self.cObject, 0 )
