...
from Definitions import dataPath, outPath, pdbDataPath, step2Path
from pBabel      import PDBFile_FromSystem, PDBFile_ToPDBModel, PDBModel_FromModelFile
from pCore       import logFile, Pickle
...
# . Get the model.
model = PDBModel_FromModelFile ( os.path.join ( step2Path, "step2.model" ) )
model.Summary ( )

# . Get its raw counterpart.
rawModel = PDBFile_ToPDBModel ( os.path.join ( dataPath, "1CDK.pdb" ) )
...
PDBFile_FromSystem ( os.path.join ( outPath, "step3.pdb" ), system )
