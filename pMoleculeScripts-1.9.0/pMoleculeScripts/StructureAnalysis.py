...
from pCore import logFile, SelfPairList_FromCoordinates3

from CrystalUtilities import CrystalExpandToP1, CrystalGetImageBondPairs
...
def IdentifyPossibleBonds ( system, atomRadii = None, bondSafetyFactor = 0.45, closeContactDistance = 0.80, excludeMMBonds = True, log = logFile ):
    ...
    if coordinates3 is not None:
        ...
        if system.coordinates3.numberUndefined <= 0:
            ...
            # . Get some options.
            if atomRadii is None: atomRadii = system.atoms.GetItemAttributes ( "covalentRadius" )
            ...
            if ( len ( possibleBonds ) > 0 ):
                ...
                for ( i, j ) in possibleBonds:
                    ...
                    if ( d > closeContactDistance ):
                        ...
                        table.Entry ( "{:6.3f}".format ( d ) )
                        table.Entry ( "" )
                if nbonds == 0: table.Entry ( "There are no possible bonds which are not also close contacts.", alignment = "c", columnSpan = 16 )
                table.Stop ( )
