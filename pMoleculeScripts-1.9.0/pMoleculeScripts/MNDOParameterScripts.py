...
from pMolecule import MNDOParameters, PeriodicTable

#===================================================================================================================================
# . Parameters for conversion between formats.
#===================================================================================================================================
# . Defined Hamiltonians.
_HAMILTONIANS = ( "am1", "mndo", "pddgmndo", "pddgpm3", "pm3", "pm6", "rm1" )
...
# . Directories.
_SCRATCH     = os.getenv ( "PDYNAMO_SCRATCH" )
_TESTPATH    = os.path.join ( _SCRATCH, "mndoParameters" )
_TEXTPATH    = "text"
_YAMLPATHIN  = os.path.join ( os.getenv ( "PDYNAMO_PARAMETERS" ), "mndoParameters" )
_YAMLPATHOUT = "yaml"
...
#===================================================================================================================================
# . Class.
class UniversalParameters ( object ):
    """Class for universal parameters."""

    defaultattributes = { "atomicNumber" :   0, \
                          "eheat"        : 0.0, \
                          "iod"          :   0, \
                          "iop"          :   0, \
                          "ios"          :   0, \
...
def MakeExtraParameters ( n, p, minhpp, toexclude ):
...
def MNDOParameters_YAMLToText ( inPath, outPath ):
    ...
    if ( norbitals > 0 ) and ( "qns" not in scalars ):
        scalars["qns"] = PeriodicTable.Element ( atomicNumber ).sqn
        if norbitals > 1: scalars["qnp"] = PeriodicTable.Element ( atomicNumber ).pqn
        if norbitals > 4: scalars["qnd"] = PeriodicTable.Element ( atomicNumber ).dqn
...
#===================================================================================================================================
def MNDOParameters_TextToYAML ( inPath, outPath, minhpp = 0.1, toexclude = None, QWRITE = True ):
    """Convert text to YAML."""
    ...
    if ( data is not None ) and ( len ( data ) > 0 ):
        ...
        atomicNumbers = data.keys ( )
        atomicNumbers.sort ( )
        for atomicNumber in atomicNumbers:
            existing   = data[atomicNumber]
            extras     = MakeExtraParameters ( atomicNumber, existing, minhpp, toexclude )
            keys       = extras.keys ( )
            keys.sort ( )
            deviations = {}
            for key in keys:
...
def MNDOParametersTextToYAML ( ):
    ...
    for hamiltonian in _HAMILTONIANS:
        ...
        for textPath in textPaths:
            MNDOParameters_TextToYAML ( textPath, outPath, minhpp = _HAMILTONIANMINHPP.get ( hamiltonian, 0.1 ), toexclude = _HAMILTONIANTOEXCLUDE.get ( hamiltonian, None ) )
