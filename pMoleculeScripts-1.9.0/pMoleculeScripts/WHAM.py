...
from pBabel import SystemSoftConstraintTrajectory            , \
...
from pCore  import Clone                                     , \
                   ...
                   Real2DArray                               , \
                   Statistics                                , \
                   UNITS_PRESSURE_ATMOSPHERES_TO_KILOJOULES_PER_MOLE
...
class WHAMObjectiveFunction ( ObjectiveFunction ):
    ...
    # . Attributes.
    clearableAttributes = { "energies"             : None ,
                            "histogram"            : None ,
                            "pmf"                  : None ,
                            "pointsPerBin"         : None ,
                            ...
                            "workE"                : None ,
                            "workP"                : None }
    defaultAttributes   = { "handler"              : None ,
                            "numberOfVariables"    : 0    ,
                            "variableOption"       : 1    }
    defaultAttributes.update ( clearableAttributes                 )
    defaultAttributes.update ( ObjectiveFunction.defaultAttributes )

    def Allocate ( self ):
        """Allocate arrays."""
        if ( self.handler is not None ) and ( self.histogram is not None ):
            self.energies      = Real1DArray.WithExtent ( self.handler.numberOfWindows ) ; self.energies.Set      ( 1.0 )
            self.probabilities = Real1DArray.WithExtent ( self.histogram.bins          ) ; self.probabilities.Set ( 1.0 )
            self.workE         = Real1DArray.WithExtent ( self.handler.numberOfWindows ) ; self.workE.Set         ( 0.0 )
            self.workP         = Real1DArray.WithExtent ( self.histogram.bins          ) ; self.workP.Set         ( 0.0 )
            self.histogram.Normalize ( self.probabilities )

    def ClearAll ( self ):
        """Clear the function."""
        for ( key, value ) in self.__class__.clearableAttributes.iteritems ( ): setattr ( self, key, value )
    ...
    def FromHandler ( selfClass, handler ):
        ...
        self.numberOfVariables = handler.numberOfWindows
        return self
    ...
    def Function ( self, variables ):
        ...
        else:                        self.energies.CopyTo ( self.workE ) ; self.workE.Ln ( )
        self.probabilities.CopyTo ( self.workP ) ; self.workP.Ln ( )
    ...
    def GetPMF ( self ):
        ...
        if ( self.reducedHistogram is not None ) and ( self.reducedProbabilities is not None ):
            pmf = Real1DArray.WithExtent ( self.reducedHistogram.bins ) ; pmf.Set ( 0.0 )
    ...
    def Initialize ( self, pressure = _Default_Pressure, temperature = _Default_Temperature ):
        ...
        if ( handler is not None ) and ( histogram is not None ):
            ...
            for ( p, t ) in zip ( handler.pressures, handler.temperatures ):
                ...
                pressures.append    ( p * UNITS_PRESSURE_ATMOSPHERES_TO_KILOJOULES_PER_MOLE )
            ...
            c = Real2DArray.WithExtents ( handler.numberOfWindows, histogram.bins ) ; c.Set ( 0.0 )
            ...
            m = Real1DArray.WithExtent ( histogram.bins          ) ; m.Set ( 0.0 )
            n = Real1DArray.WithExtent ( handler.numberOfWindows ) ; n.Set ( 0.0 )
...
#-----------------------------------------------------------------------------------------------------------------------------------
def WHAM_ConjugateGradientMinimize ( paths, **keywordArguments ):
    """Solve the WHAM equations by conjugate gradient minimization."""
    ( of, options, log ) = _InitializeSolution ( paths, ConjugateGradientMinimizer.defaultAttributes, {}, keywordArguments )
...
#-----------------------------------------------------------------------------------------------------------------------------------
def WHAM_DirectIteration ( paths, **keywordArguments ):
    ...
    ( of, options, log ) = _InitializeSolution ( paths, defaultOptions, {}, keywordArguments )
...
#-----------------------------------------------------------------------------------------------------------------------------------
def WHAM_LBFGSMinimize ( paths, **keywordArguments ):
    """Solve the WHAM equations by LBFGS minimization."""
    ( of, options, log ) = _InitializeSolution ( paths, LBFGSMinimizer.defaultAttributes, {}, keywordArguments )
...
#-----------------------------------------------------------------------------------------------------------------------------------
def WHAM_TestGradients ( paths, **keywordArguments ):
    """Test the derivatives of the WHAM equations."""
    ( of, options, log ) = _InitializeSolution ( paths, {}, {}, keywordArguments )
...
def _InitializeSolution ( paths, defaultOptions, defaultsToChange, inputOptions ):
    ...
    # . Gather all data.
    if handler is None:
        handler = SystemSoftConstraintTrajectoryDataHandler.FromTrajectoryPaths ( paths )
...
#===================================================================================================================================
def _WHAMDirectIterator ( histogram, c, m, n, f, p, fOld, pOld , **keywordArguments ):
    ...
    while ( iterations < maximumIterations ) and ( not isConverged ):
        ...
        fOld.AddScaledArray ( -1.0, f )
        pOld.AddScaledArray ( -1.0, p )
        ...
        if doPrint and ( iterations % logFrequency == 0 ):
            # . Likelihood.
            f.CopyTo ( fOld ) ; fOld.Ln ( )
            p.CopyTo ( pOld ) ; pOld.Ln ( )
