#-------------------------------------------------------------------------------
# . File      : FourierOutOfPlaneParameterContainer.py
...
class FourierOutOfPlaneParameterContainer ( MMParameterContainer ):
    ...
    defaultTermLabel = "Fourier Out-Of-Plane"
    ...
    def ProcessRawItems ( self ):
        ...
        if units is not None:
            ...
            if units.get ( "Force Constant", "" ).find ( "kcal" ) > -1:
                self.items = {}
                for ( key, fC ) in self.rawItems.iteritems ( ):
                    self.items[key] = UNITS_ENERGY_KILOCALORIES_PER_MOLE_TO_KILOJOULES_PER_MOLE * fC
            else:
