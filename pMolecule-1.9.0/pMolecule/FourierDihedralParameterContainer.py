#-------------------------------------------------------------------------------
# . File      : FourierDihedralParameterContainer.py
...
class FourierDihedralParameterContainer ( MMParameterContainer ):
    ...
    defaultTermLabel = "Fourier Dihedral"
...
#            print e[0]
    ...
    def ConvertItemUnits ( items, fCFactor, phFactor ):
        """Unit conversion."""
        newItems = {}
        for ( key, terms ) in items.iteritems ( ):
            newTerms = []
    ...
    def MakeMMTerms ( self, atomTypeLabels, termIndices ):
        ...
        if len ( termIndices ) > 0:
            ...
            for ( key, pIndices ) in parameterKeys.iteritems ( ):
    ...
    def MakeMMTermsFromConnectivity ( self, atomTypeLabels, connectivity ):
        ...
        termIndices = connectivity.GetDihedralIndices ( )
        ...
        # . Process entries.
            ...
            if units is not None:
                ...
                if units.get ( "Phase"         , "" ).find ( "degrees" ) > -1:
                    convert  = True
                    phFactor = UNITS_ANGLE_DEGREES_TO_RADIANS
            # . Wild card.
            if wildCard is None:
                items     = self.rawItems
                itemsWild = {}
