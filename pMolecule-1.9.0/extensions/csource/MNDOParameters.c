...
{
    MNDOParameters *self ;
    self = ( MNDOParameters * ) Memory_Allocate ( sizeof ( MNDOParameters ) ) ;
    /* . Counters. */
...
void MNDOParameters_CalculateOneCenterTEIs ( MNDOParameters *self )
{
    ...
    if ( self->norbitals > 0 )
    {
        ...
        self->nocteis = n ;
        MEMORY_ALLOCATEARRAY ( self->octeiindices, 4 * n, Integer16  ) ;
        MEMORY_ALLOCATEARRAY ( self->octeivalues ,     n, Real ) ;
...
MNDOParameters *MNDOParameters_Clone ( const MNDOParameters *self )
{
	...
	new->gphot = self->gphot ;
        ...
        if ( new->nocteis > 0 )
        {
            MEMORY_ALLOCATEARRAY ( new->octeiindices, 4 * new->nocteis, Integer16  ) ;
            MEMORY_ALLOCATEARRAY ( new->octeivalues ,     new->nocteis, Real ) ;
        ...
        if ( new->norbitals > 0 )
        {
            new->beta0 = Memory_Allocate_Array_Real ( new->norbitals ) ;
            new->uspd0 = Memory_Allocate_Array_Real ( new->norbitals ) ;
            new->beta  = Memory_Allocate_Array_Real ( new->norbitals ) ;
            new->uspd  = Memory_Allocate_Array_Real ( new->norbitals ) ;
        ...
        if ( new->nam1pm3g > 0 )
        {
            new->fn10 = Memory_Allocate_Array_Real ( new->nam1pm3g ) ;
            new->fn20 = Memory_Allocate_Array_Real ( new->nam1pm3g ) ;
            new->fn30 = Memory_Allocate_Array_Real ( new->nam1pm3g ) ;
            new->fn1  = Memory_Allocate_Array_Real ( new->nam1pm3g ) ;
            new->fn2  = Memory_Allocate_Array_Real ( new->nam1pm3g ) ;
            new->fn3  = Memory_Allocate_Array_Real ( new->nam1pm3g ) ;
        ...
        if ( new->ndiatomic > 0 )
        {
            new->QDIATOMICFLAGS = Memory_Allocate_Array_Boolean ( new->ndiatomic ) ;
            new->diatomica0     = Memory_Allocate_Array_Real    ( new->ndiatomic ) ;
            new->diatomicx0     = Memory_Allocate_Array_Real    ( new->ndiatomic ) ;
            new->diatomica      = Memory_Allocate_Array_Real    ( new->ndiatomic ) ;
            new->diatomicx      = Memory_Allocate_Array_Real    ( new->ndiatomic ) ;
        ...
        if ( new->npddg > 0 )
        {
            new->pddgc0 = Memory_Allocate_Array_Real ( new->npddg ) ;
            new->pddge0 = Memory_Allocate_Array_Real ( new->npddg ) ;
            new->pddgc  = Memory_Allocate_Array_Real ( new->npddg ) ;
            new->pddge  = Memory_Allocate_Array_Real ( new->npddg ) ;
...
!---------------------------------------------------------------------------------------------------------------------------------*/
/* . Disagreement in AIJL which is compensated in most cases by formula in this function.
!  . There are extra factors of sqrt ( 2 ) in case of ddp[2] and ddp[3]. */
static void MNDOParameters_FillDDPPO ( MNDOParameters *self, const Real *octeis )
{
    if ( self != NULL )
    {
...
# endif
        ...
        if ( ni > 2 )
        {
            ...
            fg = self->hpp ;
            self->ddp[2] = d ; /* . Factor of * sqrt ( 2 ) wrt MNDOD. */
            self->po [2] = POIJ ( 2, d, fg ) ;
# ifdef MNDODORBITALS
            if ( hasDOrbitals )
            {
                /* . sd. */
                da = sqrt ( 1.0e+00 / 60.0e+00 ) ;
                d  = sqrt ( aij[3] * da ) ;
                fg = octeis[19] ;
                self->ddp[3] = d ; /* . Factor of * sqrt ( 2 ) wrt MNDOD. */
