#-------------------------------------------------------------------------------
# . File      : pMolecule.CosineDihedralContainer.pyx
# . Program   : pDynamo-1.9.0                           (http://www.pdynamo.org)
# . Copyright : CEA, CNRS, Martin J. Field (2007-2014)
# . License   : CeCILL French Free Software License     (http://www.cecill.info)
#-------------------------------------------------------------------------------
"""A container for cosine dihedrals."""
...
cdef class CosineDihedralContainer ( MMTerm ):
    ...
    def __copy__ ( self ):
        """Copying."""
        cdef CosineDihedralContainer new
        new               = self.__class__.Raw ( )
        new.label         = self.label
        new.cObject       = CosineDihedralContainer_Clone ( self.cObject )
        new.isOwner       = True
    ...
    def __dealloc__ ( self ):
        """Finalization."""
        if self.isOwner:
            CosineDihedralContainer_Deallocate ( &self.cObject )
            self.isOwner = False
    ...
    def __getmodule__ ( self ):
        ...
        return "pMolecule.CosineDihedralContainer"
    ...
    def __setstate__ ( self, state ):
        ...
        for ( i, data ) in enumerate ( parameters ):
            t = len ( data )
            CosineDihedralParameter_Allocate ( &(self.cObject.parameters[i]), t )
        ...
        # . Finish processing.
        CosineDihedralContainer_FindMaximumPeriod ( self.cObject )
        CosineDihedralContainer_MakePowers        ( self.cObject )

    def _Allocate ( self, numberOfParameters, numberOfTerms ):
        """Allocation."""
        self.cObject = CosineDihedralContainer_Allocate ( numberOfTerms, numberOfParameters )
        self.isOwner = True
    ...
    def _Initialize ( self ):
        ...
        self.cObject       = NULL
        self.isOwner       = False
        self.label         = "Cosine Dihedral"
        self.parameterKeys = None

    def ActivateTerms ( self ):
        """Activate terms."""
        CosineDihedralContainer_ActivateTerms ( self.cObject )

    def DeactivateFixedAtomTerms ( self, Selection fixedatoms ):
        """Deactivate terms involving fixed atoms."""
        if fixedatoms is not None: CosineDihedralContainer_DeactivateFixedAtomTerms ( self.cObject, fixedatoms.cObject )

    def DeactivateQCAtomTerms ( self, Selection qcAtoms, Selection boundaryatoms ):
        """Deactivate terms involving QC atoms."""
        if qcAtoms is not None: CosineDihedralContainer_DeactivateQCAtomTerms ( self.cObject, qcAtoms.cObject, NULL )

    def Energy ( self, Coordinates3 coordinates3, Coordinates3 gradients3 ):
        """Energy and gradients."""
        if gradients3 is None: energy = CosineDihedralContainer_Energy ( self.cObject, coordinates3.cObject, NULL               )
        else:                  energy = CosineDihedralContainer_Energy ( self.cObject, coordinates3.cObject, gradients3.cObject )
    ...
    def Prune ( self, Selection selection, information = {} ):
        """Pruning."""
        cdef CosineDihedralContainer new
        new               = CosineDihedralContainer.Raw ( )
        new.cObject       = CosineDihedralContainer_Prune ( self.cObject, selection.cObject )
        new.isOwner       = True
        new.label         = self.label
    ...
    def Sort ( self ):
        """Sorting."""
        CosineDihedralContainer_Sort ( self.cObject )

    def SummaryEntry ( self, summary ):
        """Summary entry."""
        cdef Integer n
        if summary is not None:
            n = CosineDihedralContainer_NumberOfInactiveTerms ( self.cObject )
    ...
    def UpperBound ( self ):
        return CosineDihedralContainer_UpperBound ( self.cObject )
