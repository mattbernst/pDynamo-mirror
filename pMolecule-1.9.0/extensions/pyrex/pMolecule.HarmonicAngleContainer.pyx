#-------------------------------------------------------------------------------
# . File      : pMolecule.HarmonicAngleContainer.pyx
# . Program   : pDynamo-1.9.0                           (http://www.pdynamo.org)
# . Copyright : CEA, CNRS, Martin J. Field (2007-2014)
# . License   : CeCILL French Free Software License     (http://www.cecill.info)
#-------------------------------------------------------------------------------
"""A container for harmonic angles."""
...
cdef class HarmonicAngleContainer ( MMTerm ):
    ...
    def __copy__ ( self ):
        """Copying."""
        cdef HarmonicAngleContainer new
        new               = self.__class__.Raw ( )
        new.label         = self.label
        new.cObject       = HarmonicAngleContainer_Clone ( self.cObject )
        new.isOwner       = True
    ...
    def __dealloc__ ( self ):
        """Finalization."""
        if self.isOwner:
            HarmonicAngleContainer_Deallocate ( &self.cObject )
            self.isOwner = False
    ...
    def __getmodule__ ( self ):
        ...
        return "pMolecule.HarmonicAngleContainer"
    ...
    def __init__ ( self, numberOfParameters, numberOfTerms, label = "Harmonic Angle" ):
        """Constructor."""
        self._Initialize ( )
    ...
    def __setstate__ ( self, state ):
        ...
        # . Terms.
        for ( i, ( a1, a2, a3, p, q ) ) in enumerate ( terms ):
    ...
    def _Allocate ( self, numberOfParameters, numberOfTerms ):
        """Allocation."""
        self.cObject = HarmonicAngleContainer_Allocate ( numberOfTerms, numberOfParameters )
        self.isOwner = True
    ...
    def _Initialize ( self ):
        ...
        self.cObject       = NULL
        self.isOwner       = False
        self.label         = "Harmonic Angle"
        self.parameterKeys = None

    def ActivateTerms ( self ):
        """Activate terms."""
        HarmonicAngleContainer_ActivateTerms ( self.cObject )

    def DeactivateFixedAtomTerms ( self, Selection fixedatoms ):
        """Deactivate terms involving fixed atoms."""
        if fixedatoms is not None: HarmonicAngleContainer_DeactivateFixedAtomTerms ( self.cObject, fixedatoms.cObject )

    def DeactivateQCAtomTerms ( self, Selection qcAtoms, Selection boundaryatoms ):
        """Deactivate terms involving QC atoms."""
        if qcAtoms is not None: HarmonicAngleContainer_DeactivateQCAtomTerms ( self.cObject, qcAtoms.cObject, NULL )

    def Energy ( self, Coordinates3 coordinates3, Coordinates3 gradients3 ):
        """Energy and gradients."""
        if gradients3 is None: energy = HarmonicAngleContainer_Energy ( self.cObject, coordinates3.cObject, NULL               )
        else:                  energy = HarmonicAngleContainer_Energy ( self.cObject, coordinates3.cObject, gradients3.cObject )
    ...
    def Prune ( self, Selection selection, information = {} ):
        """Pruning."""
        cdef HarmonicAngleContainer new
        new               = HarmonicAngleContainer.Raw ( )
        new.cObject       = HarmonicAngleContainer_Prune ( self.cObject, selection.cObject )
        new.isOwner       = True
        new.label         = self.label
    ...
    def Sort ( self ):
        """Sorting."""
        HarmonicAngleContainer_Sort ( self.cObject )

    def SummaryEntry ( self, summary ):
        """Summary entry."""
        cdef Integer n
        if summary is not None:
            n = HarmonicAngleContainer_NumberOfInactiveTerms ( self.cObject )
    ...
    def UpperBound ( self ):
        return HarmonicAngleContainer_UpperBound ( self.cObject )
